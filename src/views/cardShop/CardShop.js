        ...
        return(
            ...
            <>
                ...
                </tr>
                {
                    myCard.map((item,index)=>{
                        return(
                            <>
                               <tr>
                                    <th><span className="delete" onClick={()=>this.handleDeleteCard(item)}>x</span></th>
                                    <th><img src={item.img}/></th>
                                    <th>{item.type}</th>
                                    <th>{item.money}<span> $</span></th>
                                    <th className="quantitySetUp"><span onClick={()=>this.handeOnClickDownTo(item)}>-</span><span>{item.quantity?item.quantity:1}</span><span onClick={()=>this.handleOnclickUpTo(item)}>+</span></th>
                                    <th>{item.money*(item.quantity?item.quantity:1)}<span> $</span></th>
                              </tr> 
                            </>
                        )
                    })
                ...
                <table >
                    ...
                    <tr>
                        <th>Total:</th>
                        <th>{subTotal +45}<span> $</span></th>
                    </tr>
                </table>
                </div>
                <button className="checkout" onClick={()=>this.handleOnclickCheckout()}>Check Out </button>
                <span className="money">{subTotal +45} $</span>
                </div>
                </div>
                :
                <div className="emtyCard">
                    <div className="imgemty"><img src={emtyCard}/></div>
                    <div className="textemty"><p>Your shopping cart is empty</p></div>
                    <div><button><NavLink to="/shop">Go Shopping Now</NavLink></button></div>
                </div>
            }
            </>
        )
